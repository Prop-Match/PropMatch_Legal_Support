    ...
    async def answer(
        ...
        if not relevant_docs:
            return SupportAnswer(
                id=msg_id,
                content=(
                    "عذراً، أستطيع مساعدتك فقط في الأسئلة المتعلقة بمنصة PropMatch "
                    "وكيفية استخدامها. إذا كنت ترغب في التواصل مع مسؤول الدعم، "
                    "يمكنك الضغط على زر تحويل لموظف الدعم."
                ),
                sources=[],
                declined=True,
            )
        ...
        if user_context:
            ...
            if user_context.kycRejectionReason:
                user_context_str += f"- سبب رفض التوثيق السابق: {user_context.kycRejectionReason}\n"
        system_prompt = (
            "أنت مساعد الدعم الفني الذكي لمنصة PropMatch.\n"
            f"{user_context_str}\n"
            "تعليمات هامة حول الصور والأدلة البصرية:\n"
            "- عندما تحتوي الإرشادات على دليل بصري أو صورة توضيحية، يجب عليك "
            "عرض الصورة باستخدام صيغة الماركداون المخصصة للصور فقط.\n"
            "- لا تكتب مسار الصورة أو اسم الملف أو رابطها ككود برمجي أو كنص "
            "عادي أو كرابط تشعبي. اعرض الصورة فقط دون إظهار المسار الفني.\n"
